"""
Source: https://github.com/markomanninen/md2notion/
...
Environment Variables:
    - NOTION_TOKEN: Authentication token for the Notion API.
"""
...
def search_pages_by_title(query):
    """
    Search for Notion pages by title

    :param query: The search string to perform the search
    :type markdown_text: str
    :return: A list of pages with basic properties (id, object, title, url)
    :rtype: list
    """
    notion = Client(auth=notion_key)
    ...
    for result in results:
        if result["object"] and result["object"] != "page":
            continue
        if result["archived"]:
            continue
        if result["in_trash"]:
